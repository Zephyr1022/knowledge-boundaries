...
# - Added timeout protection (10 hours max) for slower systems
# - Added GPU availability checking before execution
#
# EXPECTED EXECUTION TIMES (per layer):
# - Logistic: ~2-5 minutes (fastest)
# - MLP: ~5-15 minutes (slower due to neural network training)
# - Circular: ~8-20 minutes (slowest due to complex angular calculations)
# - LRC: ~3-8 minutes (when enabled)
#
# Execution Strategy:
...
run_system_model() {
    ...
    (
        ...
        if ! nvidia-smi -i "$GPU_ID" >/dev/null 2>&1; then
            echo "[ERROR] GPU $GPU_ID is not available or accessible" | tee -a "$SYSTEM_LOG"
            echo "[ERROR] Skipping $SYSTEM for $MODEL_DISPLAY on GPU $GPU_ID" | tee -a "$SYSTEM_LOG"
            return 1
        fi
...
wait_for_processes() {
    ...
    echo "[INFO] Note: MLP and Circular systems may take longer due to complexity"
    
    # Wait for all processes with progress monitoring
    local RUNNING=1
    local ITERATION=0
    local MAX_ITERATIONS=1200  # 10 hours max (1200 * 30s = 10h)
    
    while [ $RUNNING -eq 1 ] && [ $ITERATION -lt $MAX_ITERATIONS ]; do
        RUNNING=0
        ITERATION=$((ITERATION + 1))
        echo -n "[$(date '+%H:%M:%S')] Status (iter ${ITERATION}/${MAX_ITERATIONS}): "
        ...
        if [ $RUNNING -eq 1 ]; then
            # Check more frequently for better monitoring
    ...
    if [ $ITERATION -ge $MAX_ITERATIONS ]; then
        echo ""
        echo "[WARNING] Timeout reached after 10 hours. Some processes may still be running."
        echo "[WARNING] Check GPU status and manually terminate if needed."
        
        # Show which processes are still running
        echo "[INFO] Still running processes:"
        for i in "${!PIDS[@]}"; do
            PID="${PIDS[$i]}"
            NAME="${PIDS_NAMES[$i]}"
            if kill -0 "$PID" 2>/dev/null; then
                echo "  $NAME (PID: $PID) - Consider manual termination"
            fi
        done
    fi
    
    echo ""
    echo "[COMPLETE] All processes finished or timeout reached at $(date '+%Y-%m-%d %H:%M:%S')"
}
...
echo ""
echo "Expected timing per layer (varies by system complexity):"
echo "  Logistic: ~2-5 min | MLP: ~5-15 min | Circular: ~8-20 min"
echo "======================================================================"
